...
enough_battery2(FacilityAux, FacilityId1, FacilityId2, Result) :- role(Role, Speed, _, _, _) & pucrs.agentcontest2016.actions.route(Role, FacilityAux, FacilityId1, RouteLen1) & pucrs.agentcontest2016.actions.route(Role, FacilityId1, FacilityId2, RouteLen2) & charge(Battery) & ((Battery > ((RouteLen1 / Speed * 10) + (RouteLen2 / Speed * 10) + 10) & Result = true) | (Result = false)).
...
enough_battery_charging2(FacilityId, Result, Battery) :- role(Role, Speed, _, _, _) & pucrs.agentcontest2016.actions.route(Role, FacilityId, RouteLen) & ((Battery > ((RouteLen / Speed * 10) + 10) & Result = true) | (Result = false)).
